...
#define FREEIMAGE_COLORORDER FREEIMAGE_COLORORDER_RGB
	...
	//---------------------------------------------------------------------
	FIBITMAP* FreeImageCodec::encode(MemoryDataStreamPtr& input, BWCodecDataPrt& pData) const
	{
		// Set error handler
		...
		BWPixelBox src(pImgData->width ,pImgData->height, pImgData->depth, pImgData->format, input->getPtr());
		...
		BWPixelBox convBox(pImgData->width, pImgData->height, 1, requiredFormat);
		if (requiredFormat != pImgData->format)
		{
			conversionRequired = true;
			// Allocate memory
			convBox.mData = new unsigned char[convBox.getConsecutiveSize()];  //OGRE_ALLOC_T(unsigned char, convBox.getConsecutiveSize(), MEMCATEGORY_GENERAL);
			// perform conversion and reassign source
			BWPixelBox newSrc(pImgData->width, pImgData->height, 1, pImgData->format, input->getPtr());
			PixelUtil::bulkPixelConversion(newSrc, convBox);
			srcData = static_cast<unsigned char*>(convBox.mData);
		}
    ...
    //---------------------------------------------------------------------
	BWDataStreamPrt FreeImageCodec::code(MemoryDataStreamPtr& input, BWCodec::BWCodecDataPrt& pData) const
    {        
		// Set error handler
		FreeImage_SetOutputMessage(FreeImageSaveErrorHandler);
		...
		//
		 BWDataStreamPrt outstream(new  MemoryDataStream(ourData, size, true));
	...
	void FreeImageCodec::codeToFile(MemoryDataStreamPtr& input,
        const std::string& outFileName, BWCodec::BWCodecDataPrt& pData) const
    {
		// Set error handler
		FreeImage_SetOutputMessage(FreeImageSaveErrorHandler);
    ...
    //---------------------------------------------------------------------
    BWCodec::DecodeResult FreeImageCodec::decode(BWDataStreamPrt& input) const
    {
		// Set error handler
		FreeImage_SetOutputMessage(FreeImageLoadErrorHandler);
		...
		imgData->num_mipMap = 0; // no mipmaps in non-DDS 
        imgData->flag = 0;
		...
		unsigned char* pSrc;
		unsigned char* pDst = output->getPtr();
        ...
        DecodeResult ret;
        ret.first = output;
		ret.second = BWCodecDataPrt(imgData);
		return ret;
