...
#include <memory>
    ...
    */
    ...
    //-----------------------------------------------------------------------
    PixelFormat PixelUtil::getFormatForBitDepths(PixelFormat fmt, unsigned short integerBits, unsigned short floatBits)
    {
        switch (integerBits)
        {
        case 16:
    ...
    */
    ...
    //-----------------------------------------------------------------------
    void PixelUtil::packColour(const unsigned char r, const unsigned char g, const unsigned char b, const unsigned char a, const PixelFormat pf,  void* dest)
    {
    ...
    //-----------------------------------------------------------------------
    void PixelUtil::unpackColour(unsigned char *r, unsigned char *g, unsigned char *b, unsigned char *a, PixelFormat pf,  const void* src)
    {
        const PixelFormatDescription &des = getDescriptionFor(pf);
        if(des.flags & PFF_NATIVEENDIAN) {
    ...
    {
        BWPixelBox src(count, 1, 1, srcFormat, srcp),
				 dst(count, 1, 1, dstFormat, destp);

        bulkPixelConversion(src, dst);
    }
    //-----------------------------------------------------------------------
    void PixelUtil::bulkPixelConversion(const BWPixelBox &src, const BWPixelBox &dst)
    {
		...
		if(PixelUtil::isCompressed(src.mPixelFormat) || PixelUtil::isCompressed(dst.mPixelFormat))
		{
			if(src.mPixelFormat == dst.mPixelFormat)
			{
				memcpy(dst.mData, src.mData, src.getConsecutiveSize());
				return;
			}
			else
			{
        ...
        if(src.mPixelFormat == dst.mPixelFormat) {
            ...
            const size_t srcPixelSize = PixelUtil::getNumElemBytes(src.mPixelFormat);
            const size_t dstPixelSize = PixelUtil::getNumElemBytes(dst.mPixelFormat);
            unsigned char *srcptr = static_cast<unsigned char*>(src.mData)
                + (src.left + src.top * src.mRowPitch + src.front * src.mSlicePitch) * srcPixelSize;
            unsigned char *dstptr = static_cast<unsigned char*>(dst.mData)
				+ (dst.left + dst.top * dst.mRowPitch + dst.front * dst.mSlicePitch) * dstPixelSize;

            // Calculate pitches+skips in bytes
            const size_t srcRowPitchBytes = src.mRowPitch*srcPixelSize;
            //const size_t srcRowSkipBytes = src.getRowSkip()*srcPixelSize;
            const size_t srcSliceSkipBytes = src.getSliceSkip()*srcPixelSize;

            const size_t dstRowPitchBytes = dst.mRowPitch*dstPixelSize;
		...
		if (dst.mPixelFormat == PF_X8R8G8B8 || dst.mPixelFormat == PF_X8B8G8R8)
		{
			// Do the same conversion, with PF_A8R8G8B8, which has a lot of
			// optimized conversions
			BWPixelBox tempdst = dst;
			tempdst.mPixelFormat = dst.mPixelFormat==PF_X8R8G8B8?PF_A8R8G8B8:PF_A8B8G8R8;
		...
		if((src.mPixelFormat == PF_X8R8G8B8||src.mPixelFormat == PF_X8B8G8R8) && !hasAlpha(dst.mPixelFormat))
		{
			// Do the same conversion, with PF_A8R8G8B8, which has a lot of
			// optimized conversions
			BWPixelBox tempsrc = src;
			tempsrc.mPixelFormat = src.mPixelFormat == PF_X8R8G8B8 ? PF_A8R8G8B8 : PF_A8B8G8R8;
...
// NB VC6 can't handle the templates required for optimised conversion, tough
        ...
        for(size_t z=src.front; z<src.back; z++)
        {
            for(size_t y=src.top; y<src.bottom; y++)
            {
                ...
                {
                    unpackColour(&r, &g, &b, &a, src.mPixelFormat, srcptr);
                    packColour(r, g, b, a, dst.mPixelFormat, dstptr);
                    srcptr += srcPixelSize;
                    dstptr += dstPixelSize;
                }
