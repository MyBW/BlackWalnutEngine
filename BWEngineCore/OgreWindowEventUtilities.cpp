...
#include "../BWEngine/ListenerTest.h"
...
ListenerTest* WindowEventUtilities::listenerTest = NULL;
//--------------------------------------------------------------------------------//
...
//--------------------------------------------------------------------------------//
void WindowEventUtilities::addWindowEventListener( BWRenderWindow* window, WindowEventListener* listener )
{
	_msListeners.insert(std::make_pair(window, listener));
}

//--------------------------------------------------------------------------------//
void WindowEventUtilities::removeWindowEventListener( BWRenderWindow* window, WindowEventListener* listener )
{
...
//--------------------------------------------------------------------------------//
void WindowEventUtilities::_addRenderWindow(BWRenderWindow* window)
{
	_msWindows.push_back(window);
}

//--------------------------------------------------------------------------------//
void WindowEventUtilities::_removeRenderWindow(BWRenderWindow* window)
{
...
void WindowEventUtilities::_addListenerTest(ListenerTest * listenertest)
{
	WindowEventUtilities::listenerTest = listenertest;
}
#if OGRE_PLATFORM == OGRE_PLATFORM_WIN32
//--------------------------------------------------------------------------------//
LRESULT CALLBACK WindowEventUtilities::_WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	BWRenderWindow* win = (BWRenderWindow*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
	if (!win)
	...
	case WM_ACTIVATE:
	{
        ...
        {
			if (win->isDeactivateOnFocusChange())
            {
    		    win->setActive( false );
            }
        }
	...
	case WM_LBUTTONDOWN:
		...
		{
		case MK_LBUTTON:
			listenerTest->SetLeftButton(true);
			listenerTest->SetMousePosition(LOWORD(lParam), HIWORD(lParam));
			listenerTest->SetOldPosition(LOWORD(lParam), HIWORD(lParam));
			break;
		default:
			break;
		}
	...
	case WM_MOUSEMOVE:
	{
		  if (listenerTest->isLeftButtonDown())
		  {
			  listenerTest->SetMousePosition(LOWORD(lParam), HIWORD(lParam));
			  listenerTest->Update();
		  }
	...
	case WM_LBUTTONUP:
	{
						 listenerTest->SetLeftButton(false);
						 break;
	}
	case WM_KEYDOWN:
		listenerTest->SetKeyCode(wParam);
		listenerTest->Update();
		break;
	case WM_SYSKEYDOWN:
		switch( wParam )
