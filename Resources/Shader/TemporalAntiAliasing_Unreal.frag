...
void main()
{
	...
	vec3 MinDepthPos;
	const float ExposureScale = 0.006;
   ...
   if(IsDynamic)//当场景是动态的时候 使用Velocity 来表示采样 这里IsDynamic一直会生效 因为Jitter的存在
   {
       BackVec = DecodeVelocityFormTexture(Velocity); // Unreal中使用了一种编码方式 使得使用更少的位数来存储Veclocity
   } 
   ...
   float NeighborWeight1 = HdrWeightY(Neighbor1.x , ExposureScale) ;
   float NeighborWeight3 = HdrWeightY(Neighbor3.x , ExposureScale) ;
   float NeighborWeight4 = HdrWeightY(Neighbor4.x , ExposureScale) ;
   float NeighborWeight5 = HdrWeightY(Neighbor5.x , ExposureScale) ;
   float NeighborWeight7 = HdrWeightY(Neighbor7.x , ExposureScale) ;
   ...
   float HistoryWidget = HdrWeightY(HistoryColor.x , ExposureScale) ;
   HistoryColor *= HistoryWidget ;
   
   
   /*vec2 TempSample = textureCoord.xy * 2 - vec2(1.0) - Velocity.xy  ;
   TempSample = TempSample * vec2(0.5) + 0.5 ;
   HistoryColor = texture2D(HistoryRT , TempSample.xy) ;
   
   HistoryColor.rgb = YCoCgToRGB(HistoryColor.rgb) ;
   Filterd.rgb = YCoCgToRGB(Filterd.rgb) ;
*/
   ...
   vec4  OutColor ;
   OutColor.rgb = HistoryColor.rgb *( 1.0 - BlendFinal) + Filterd.rgb * BlendFinal ;
   OutColor.a = max(HistoryColor.a , 1.0/2.0);
   OutColor.rgb *= HdrWeightY(OutColor.x , ExposureScale);
   OutColor.rgb = YCoCgToRGB(OutColor.rgb);
   OutColor.rgb = -min(-OutColor.rgb, 0.0) ;
   OutColor.a = IsDynamic4 ? 1.0 : 0.0;
   FinalAAResult = OutColor ;
}